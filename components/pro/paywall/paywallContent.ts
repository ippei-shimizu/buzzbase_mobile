...
/**
 * 価値画面で訴求する機能ブロックのまとまり。トリガーと同じ系統の機能を並べて、
 * 「いま見ていた画面の延長線上で何ができるようになるか」を途切れさせない。
 */
const VALUE_BLOCK_FAMILIES: readonly ProFeature[][] = [
  // 成績分析: 打席データを持つユーザーが最も多く、加入直後に効果が出る系統。
  [
    "hit_direction_average",
    "count_situation_average",
    "pitch_type_average",
...
// トリガーがどの系統にも属さないとき（general / グループ / 広告など）に見せる既定の3つ。
// 打席データを持つユーザーが最も多い成績分析を先頭に置く。
const DEFAULT_VALUE_BLOCKS: readonly ProFeature[] = [
  "hit_direction_average",
  "count_situation_average",
  "no_ads",
];

export const VALUE_BLOCK_LIMIT = 3;

/**
 * 価値画面に並べる機能ブロックを決める。ヒーローで見せているトリガー自身は除外し、
 * 同じ系統の機能を最大3件返す。系統が見つからないトリガーは既定の3件にフォールバックする。
 *
 * @param trigger Paywall を開いた起点の機能キー（`general` を含む）
 * @returns 表示する Pro 機能キー（最大 `VALUE_BLOCK_LIMIT` 件）
 */
export function valueBlocksFor(trigger: ProTrigger): ProFeature[] {
  const family = VALUE_BLOCK_FAMILIES.find(
    (keys) => trigger !== "general" && keys.includes(trigger),
  );
  const candidates = family ?? DEFAULT_VALUE_BLOCKS;
  return candidates
    .filter((key) => key !== trigger)
    .slice(0, VALUE_BLOCK_LIMIT);
}
