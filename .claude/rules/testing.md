...
- 落ちない場合、同じ目的の防御が別の層にも入っていて、そちらだけで通っている可能性を疑う。その層が持たない固有の振る舞いを対象に書き直す
- 同じ assertion を複数の層に置かない
- **「出ないこと」を見る assertion（`queryByText(...)` が null）は、戻した実装で実際に描画される文字列を比較対象にする**。例: `formatRate(0)` は `".000"` ではなく `"0.000"` を返すため、`queryByText(".000")` は実装が壊れていても通る
