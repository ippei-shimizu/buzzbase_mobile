...
- 戻したときの確認は**スイート全体ではなく、追加したテスト1件ずつ**で行う。旧実装でも「別の理由で同じ結果」になるテストは、スイートが落ちていても紛れ込む（例: 消化済みマイルストーンの再要求を見るつもりが、旧実装でも「その値はマイルストーンではない」で同じく不発になる）
- 落ちない場合、同じ目的の防御が別の層にも入っていて、そちらだけで通っている可能性を疑う。その層が持たない固有の振る舞いを対象に書き直す（例: 「広告を出さない」を見るなら、広告側の猶予期間を明けた状態を seed しないと常に出ない）
- **テスト間で残る状態を疑う**。`jest.clearAllMocks()` は呼び出し履歴だけを消し、`mockImplementation` は残す（`jest.config.js` に `resetMocks` は無い）。フックのモジュールスコープの値（`useReviewPrompt` の `sessionTriggeredKeys` など）もリセットされない。後続のテストがこれを引き継ぐと「別の理由で同じ結果」になるため、差し替えた実装は `afterEach` で戻し、モジュールスコープに残る値で2回目の発火を見るテストは1件にまとめる
- 同じ assertion を複数の層に置かない
- **「出ないこと」を見る assertion（`queryByText(...)` が null）は、戻した実装で実際に描画される文字列を比較対象にする**。例: `formatRate(0)` は `".000"` ではなく `"0.000"` を返すため、`queryByText(".000")` は実装が壊れていても通る。同じ PR で削除したコンポーネントの文言も、戻した実装には存在しないので比較対象にならない
- **「リクエストが飛ばないこと」を見る assertion（遅延取得など）は、render 直後に同期で確認しない**。MSW の resolver は次の tick 以降に走るため、マウント時に取得する実装に戻しても通る。`act` 内で短く待ってから確認する（`await waitFor(() => expect(requested).toBe(false))` は初回で成立して即 return するので待機にならない）
