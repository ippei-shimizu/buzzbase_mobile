...
// テスト間で configured フラグをリセットするため、毎回 jest.resetModules() で
// revenueCatService モジュールを再評価し、初期状態（configured = false）から始める。
const loadService = (): RevenueCatService => {
  jest.resetModules();
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  return require("../revenueCatService") as RevenueCatService;
};
